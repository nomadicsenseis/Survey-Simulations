...
def calculate_SHAP_and_probability_binary(model_promoter, model_detractor, df):
    ...
    id_df = df[['respondent_id', 'date_flight_local','otp15_takeoff','promoter_binary', 'detractor_binary', 'monthly_weight','cluster']]
    
    # Preparar el conjunto de datos para predicciones, excluyendo ID y fechas
    test_set = df.drop(['respondent_id', 'date_flight_local','otp15_takeoff','promoter_binary', 'detractor_binary','monthly_weight', 'cluster'], axis=1, errors='ignore')
...
def from_shap_to_probability_binary(df, features_dummy, label_binary):
    ...
    relevant_columns = ['respondent_id', 'date_flight_local','otp15_takeoff'] + shap_columns + ['promoter_binary', 'detractor_binary', 'monthly_weight','cluster'] + [f'base_prob{class_suffix}'] + features_dummy
...
def from_shap_to_probability_binary(df, features_dummy, label_binary):
    ...
    relevant_columns = ['respondent_id', 'date_flight_local','otp15_takeoff'] + shap_columns + ['promoter_binary', 'detractor_binary', 'monthly_weight','cluster'] + [f'base_prob{class_suffix}', out_prob_col] + features_dummy
    print(output_df)
    output_df = output_df[relevant_columns]
