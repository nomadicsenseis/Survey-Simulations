...
if __name__ == "__main__":
    ...
    df_predict_sampled = read_csv(f"s3://{S3_BUCKET}/{read_path_ori}/sampled_clusterized.csv")
    ...
    df_probabilities_sampled = utils.predict_and_explain(clf_model[model_names[0]], clf_model[model_names[1]], df_predict_sampled, features, K_uncertainty=5)

    # Añadir la columna 'simulation_client_type' a cada DataFrame
    df_probabilities_ori['simulation_client_type'] = 'original' 
    df_probabilities_sampled['simulation_client_type'] = 'original' 

    # Save the prediction results to S3
    save_path_ori = f"s3://{S3_BUCKET}/{S3_PATH_WRITE}/015_predict_explain_ori_step/{year}{month}{day}/predictions.csv"
    save_path_sampled = f"s3://{S3_BUCKET}/{S3_PATH_WRITE}/015_predict_explain_ori_step/{year}{month}{day}/sampled_predictions.csv"
    SAGEMAKER_LOGGER.info("userlog: Saving information for predict step in %s.", save_path_sampled)
    df_probabilities_ori.to_csv(save_path_ori, index=False)
    df_probabilities_sampled.to_csv(save_path_sampled, index=False)